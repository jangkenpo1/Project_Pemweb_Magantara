...
<head>
    ...
    <title>@yield('title', 'Dashboard') — MaganTara</title>
    <meta name="description" content="@yield('meta_description', 'Dashboard Mahasiswa MaganTara')">
...
<body style="background: var(--color-fog); min-height: 100vh;">
        ...
        <div class="navbar-inner">
            <div style="display: flex; align-items: center; gap: 32px;">
                ...
                <div style="display: flex; gap: 4px;">
                    <a href="/lowongan" class="nav-link {{ request()->is('lowongan*') ? 'active' : '' }}">Cari Magang</a>
                    <a href="/mahasiswa/wishlist" class="nav-link {{ request()->is('mahasiswa/wishlist*') ? 'active' : '' }}">Wishlist</a>
                    <a href="/mahasiswa/lamaran" class="nav-link {{ request()->is('mahasiswa/lamaran*') ? 'active' : '' }}">Lamaran Saya</a>
                </div>
            </div>
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div style="position: relative;" x-data="{ open: false }">
                    <button @click="open = !open" style="display: flex; align-items: center; gap: 8px; background: none; border: none; cursor: pointer; padding: 4px;">
                        @if(auth('mahasiswa')->user()->avatar)
                            <img src="{{ Storage::url(auth('mahasiswa')->user()->avatar) }}" alt="Avatar" class="avatar">
                        @else
                            <div class="avatar-initials">{{ strtoupper(substr(auth('mahasiswa')->user()->name, 0, 1)) }}</div>
                        @endif
                        <span style="font-size: 14px; font-weight: 500; color: var(--color-ash);">{{ auth('mahasiswa')->user()->name }}</span>
    ...
    <main>
        @if(session('success'))
            <div style="background: #dcfce7; border-bottom: 1px solid #bbf7d0; padding: 12px 24px; text-align: center; font-size: 14px; color: #15803d;">{{ session('success') }}</div>
        @endif
        @if(session('error'))
            <div style="background: #fee2e2; border-bottom: 1px solid #fecaca; padding: 12px 24px; text-align: center; font-size: 14px; color: #991b1b;">{{ session('error') }}</div>
        @endif
        @yield('content')
    </main>
