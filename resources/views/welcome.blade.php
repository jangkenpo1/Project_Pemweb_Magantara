            ...
            <div style="max-width: 780px; margin: 0 auto; text-align: center;">
                ...
                <div style="display: flex; align-items: center; justify-content: center; gap: 80px; margin-top: 64px; padding-top: 40px; border-top: 1px solid rgba(163,166,175,0.3); animation: fadeIn 0.6s ease 0.5s both;">
                    <div style="text-align: center;">
                        <p class="stat-number counter" data-target="{{ $stats['mahasiswa'] }}" style="font-size: 32px; margin-bottom: 8px;">0</p>
                    ...
                    <div style="text-align: center;">
                        <p class="stat-number counter" data-target="{{ $stats['perusahaan'] }}" style="font-size: 32px; margin-bottom: 8px;">0</p>
                        <p class="stat-label" style="font-size: 11px; letter-spacing: 0.05em;">Perusahaan Mitra</p>
                    </div>
                    <div style="width: 1px; height: 50px; background: rgba(163,166,175,0.4);"></div>
                    <div style="text-align: center;">
                        <p class="stat-number counter" data-target="{{ $stats['lowongan'] }}" style="font-size: 32px; margin-bottom: 8px;">0</p>
    ...
    <section style="padding: 24px 0 48px;">
        <div class="page-container">
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; justify-content: center;">
                ...
                @foreach(['UI/UX Design', 'React Developer', 'Data Analyst', 'Marketing Digital', 'Business Development', 'Mobile Developer', 'Backend Developer', 'Content Creator'] as $tag)
                    <a href="/lowongan?q={{ urlencode($tag) }}" class="filter-pill">{{ $tag }}</a>
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section style="padding: 96px 0;">
        <div class="page-container">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px;">
                @forelse($latestLowongan as $job)
                    <a href="/lowongan/{{ $job->id }}" class="job-card card-hover">
                        <div style="display: flex; align-items: flex-start; gap: 14px; margin-bottom: 16px;">
                            <div style="width: 44px; height: 44px; border-radius: var(--radius-image); background: white; border: 1px solid rgba(163,166,175,0.3); display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0;">
                                @if($job->perusahaan->logo)
                                    <img src="{{ Storage::url($job->perusahaan->logo) }}" alt="Logo {{ $job->perusahaan->name }}" style="width: 100%; height: 100%; object-fit: contain; padding: 4px;">
                                @else
                                    <span style="font-weight: 700; color: var(--color-ink); font-size: 16px;">{{ strtoupper(substr($job->perusahaan->name, 0, 1)) }}</span>
                                @endif
                            </div>
                            <div style="flex: 1; min-width: 0;">
                                <h3 style="font-size: 15px; font-weight: 600; color: var(--color-ink); margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $job->title }}</h3>
                                <p style="font-size: 13px; color: var(--color-graphite);">{{ $job->perusahaan->name }}</p>
                            </div>
                        </div>
                        <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 16px;">
                            <span class="badge {{ strtolower($job->work_system) === 'remote' ? 'badge-sky' : 'badge-fog' }}">
                                {{ $job->work_system_label }}
                            </span>
                            <span class="badge {{ strtolower($job->payment_type) === 'paid' ? 'badge-apricot' : 'badge-fog' }}">
                                {{ $job->payment_type_label }}
                            </span>
                            <span class="badge badge-fog">{{ $job->duration_months }} Bulan</span>
                        </div>
                        <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                            ...
                            @if($job->skills->count() > 3)
                                <span style="font-size: 11px; color: var(--color-graphite); background: var(--color-fog); padding: 3px 8px; border-radius: 6px;">+{{ $job->skills->count() - 3 }}</span>
                            @endif
                        </div>
                        <div style="display: flex; align-items: center; gap: 4px; margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(163,166,175,0.2);">
                            ...
                            <span style="font-size: 12px; color: var(--color-graphite);">{{ $job->city ? $job->city->name : 'Indonesia' }}</span>
                        </div>
                    </a>
                @empty
        ...
        <div class="page-container">
            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 48px; margin-bottom: 40px;">
                ...
                <div>
                    ...
                    @foreach(['Cari Lowongan', 'Daftar', 'Masuk', 'Wishlist'] as $link)
                        <a href="#" style="display: block; font-size: 13px; color: rgba(163,166,175,0.8); text-decoration: none; margin-bottom: 8px; transition: color 0.15s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(163,166,175,0.8)'">{{ $link }}</a>
                    @endforeach
                </div>
                <div>
                    ...
                    @foreach(['Pasang Lowongan', 'Daftar Perusahaan', 'Masuk', 'Verifikasi'] as $link)
                        <a href="#" style="display: block; font-size: 13px; color: rgba(163,166,175,0.8); text-decoration: none; margin-bottom: 8px; transition: color 0.15s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(163,166,175,0.8)'">{{ $link }}</a>
                    @endforeach
                </div>
                <div>
                    ...
                    @foreach(['Tentang Kami', 'Kebijakan Privasi', 'Syarat & Ketentuan', 'Hubungi Kami'] as $link)
                        <a href="#" style="display: block; font-size: 13px; color: rgba(163,166,175,0.8); text-decoration: none; margin-bottom: 8px; transition: color 0.15s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(163,166,175,0.8)'">{{ $link }}</a>
                    @endforeach
                </div>
            </div>
