...
<div class="page-container" style="padding-top: 40px; padding-bottom: 60px;">
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 40px;" class="stagger-children">
        ...
        <div class="stat-card animate-fade-in-up" style="padding: 20px;">
            ...
            <p class="stat-number" style="font-size: 28px;">{{ $totalLamaran }}</p>
        ...
        <div class="stat-card animate-fade-in-up" style="padding: 20px;">
            ...
            <p class="stat-number" style="font-size: 28px;">{{ $lamaranProses }}</p>
        ...
        <div class="stat-card animate-fade-in-up" style="padding: 20px;">
            ...
            <p class="stat-number" style="font-size: 28px;">{{ $lamaranDiterima }}</p>
        ...
        <div class="stat-card animate-fade-in-up" style="padding: 20px;">
            ...
            <p class="stat-number" style="font-size: 28px;">{{ $totalBookmark }}</p>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
            ...
            @forelse($lamarans as $lamaran)
                <div class="applicant-row" style="margin-bottom: 12px;">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <p style="font-size: 14px; font-weight: 500; color: var(--color-ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $lamaran->lowongan->title }}</p>
                        <p style="font-size: 12px; color: var(--color-graphite);">{{ $lamaran->lowongan->perusahaan->name }}</p>
                    </div>
                    <span class="badge {{ 'status-' . $lamaran->status }}" style="font-size: 11px; padding: 3px 10px;">{{ $lamaran->status_label }}</span>
                </div>
            @empty
        ...
        <div class="card">
            ...
            @forelse($lowonganTerbaru as $lowongan)
                <a href="/lowongan/{{ $lowongan->id }}" style="display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(163,166,175,0.2); text-decoration: none;" onmouseover="this.style.background='var(--color-fog)'" onmouseout="this.style.background='transparent'">
                    <div class="company-logo" style="display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; color: var(--color-ink); overflow: hidden;">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        <p style="font-size: 14px; font-weight: 500; color: var(--color-ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $lowongan->title }}</p>
                        <p style="font-size: 12px; color: var(--color-graphite);">{{ $lowongan->perusahaan->name }}</p>
                    </div>
                    <span class="badge {{ $lowongan->payment_type === 'paid' ? 'badge-apricot' : 'badge-fog' }}" style="font-size: 11px; padding: 2px 8px;">{{ $lowongan->payment_type_label }}</span>
                </a>
            @empty
