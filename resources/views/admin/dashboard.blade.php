...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 32px;">
    <div class="stat-card">
        ...
        <p class="stat-number" style="font-size: 32px;">{{ $stats['total_mahasiswa'] }}</p>
    ...
    <div class="stat-card">
        ...
        <p class="stat-number" style="font-size: 32px;">{{ $stats['total_perusahaan'] }}</p>
    ...
    <div class="stat-card">
        ...
        <p class="stat-number" style="font-size: 32px;">{{ $stats['total_lowongan'] }}</p>
    ...
    <div class="stat-card">
        ...
        <p class="stat-number" style="font-size: 32px;">{{ $stats['total_lamaran'] }}</p>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
    ...
    <div class="card">
        ...
        @if($stats['pending_verify'] > 0)
            <div style="background: #fef9c3; border-radius: 12px; padding: 12px 16px; margin-bottom: 16px; font-size: 13px; color: #854d0e;">
                Terdapat <strong>{{ $stats['pending_verify'] }}</strong> perusahaan menunggu verifikasi.
            </div>
        @endif
        
        @forelse($perusahaanPending as $p)
            <div style="display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(163,166,175,0.2);">
                ...
                <div style="flex: 1; min-width: 0;">
                    <a href="/admin/perusahaan/{{ $p->id }}" style="font-size: 14px; font-weight: 500; color: var(--color-ink); text-decoration: none;">{{ $p->name }}</a>
                    <p style="font-size: 12px; color: var(--color-graphite);">{{ $p->industry->name ?? 'Belum ada industri' }}</p>
                </div>
                <form method="POST" action="/admin/perusahaan/{{ $p->id }}/verify" style="display: inline-block;">
                    @csrf
    ...
    <div class="card">
        ...
        @forelse($lowonganTerbaru as $lowongan)
            <div style="display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(163,166,175,0.2);">
                <div style="flex: 1; min-width: 0;">
                    <p style="font-size: 14px; font-weight: 500; color: var(--color-ink);">{{ $lowongan->title }}</p>
